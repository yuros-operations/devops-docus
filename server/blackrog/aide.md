...
```
sudo pacman -S  pkgconf bison gcc pcre flex wget
```
```
sudo pkg-config --libs --cflags glib-2.0
```
```
cd /opt
```
```
sudo wget https://github.com/aide/aide/releases/download/v0.19.2/aide-0.19.2.tar.gz 
```
```
...
```
sudo nvim /etc/systemd/system/aide.service
```
```
[Unit]
...
```
```
sudo nvim /etc/systemd/system/aide.timer
```
```
[Unit]
