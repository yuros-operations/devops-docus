...
```
echo "#blacksky initrams" > /etc/mkinitcpio.d/blacksky.conf
```
...
```
printf "MODULES=($CPIOMODS)\nBINARIES=($CPIOBINS)\nFILES=()\nHOOKS=($CPIOHOOK)" >> /etc/mkinitcpio.d/blacksky.conf 
```
...
```
echo "#blacksky linux preset" > /etc/mkinitcpio.d/linux-hardened.preset
```

```
echo 'ALL_config="/etc/mkinitcpio.d/blacksky.conf"' >> /etc/mkinitcpio.d/linux-hardened.preset
```
...
```
echo 'default_uki="/boot/efi/linux/blacksky.efi"' >> /etc/mkinitcpio.d/linux-hardened.preset
```
